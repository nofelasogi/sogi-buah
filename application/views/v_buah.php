...
<table class="table table-hover table-stripped">
	...
	<thead>
		...
		<tr>
			...
			<td>no</td><td>kode_buah</td><td>buah</td><td>tahun</td><td>kategori</td><td>harga</td><td>cover</td><td>stok</td><td></td><td></td>
	...
	<tbody>
		...
		<tr>
			...
			<td><?=$no?></td><td><?=$bk->kode_buah?></td><td><?=$bk->buah?></td><td><?=$bk->tahun?></td><td><?=$bk->nama_kategori?></td><td><?=$bk->harga?></td><td><img src="<?=base_url('assets/gambar/'.$bk->cover)?>" style="width:40px;"></td><td><?=$bk->stok?></td>

			<td><?php if($this->session->userdata('level')=="admin"){?> <a href="#ubah" data-toggle="modal" onclick="edit(<?=$bk->kode_buah?>)"  class="btn btn-warning">Ubah</a><?php }else{ 		echo "anda kasir"; }?></td>

			<td><?php if($this->session->userdata('level')=="admin"){?><a href="<?=base_url('index.php/buah/hapus/'.$bk->kode_buah)?>" onclick="return confirm('apakah anda yakin untuk menghapus')" class="btn btn-danger">Hapus</a><?php }else{ echo "anda kasir"; }?></td>
			...
			<div class="modal-body">
				...
				<form action="<?=base_url('index.php/buah/tambah')?>" method="post" enctype="multipart/form-data">
					...
					<table>
						...
						<tr>
							<td>Judul Buah</td>
							<td><input type="text" name="buah" style="margin-left: 20px;"></td>
						</tr>
			...
			<div class="modal-body">
				...
				<form action="<?=base_url('index.php/buah/update')?>" method="post" enctype="multipart/form-data">
					...
					<table>
						...
						<tr>
							<td>judul buah</td>
							<td><input type="text" name="buah"  required  id="buah" style="margin-left: 20px;"></td>
						</tr>
